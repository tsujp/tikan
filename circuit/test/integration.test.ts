...
async function foo () {
    ...
    const { witness: white_witness, returnValue: white_returnValue } = await white_noir
        .execute(white_pawn_g2_g4)
    console.log('done')
    ...
    const { witness: black_witness, returnValue: black_returnValue } = await black_noir
        .execute(black_pawn_g7_g5)
    console.log('done')
